...
<div class="mb-8">
    ...
    <div class="flex items-center gap-1 bg-gray-100/50 dark:bg-white/5 p-1 rounded-xl w-fit border border-gray-200 dark:border-white/10">
        <a href="profile.php" class="px-6 py-2.5 rounded-lg text-sm font-bold transition-all bg-white dark:bg-white/10 text-primary shadow-sm">
            Profile Info
        </a>
        <a href="account.php" class="px-6 py-2.5 rounded-lg text-sm font-bold transition-all text-gray-500 dark:text-gray-400 hover:text-primary">
            Security & Account
        </a>
    </div>
</div>
...
<?php if (!empty($success)): ?>
    <div class="mb-6 p-4 bg-green-50 dark:bg-green-900/20 text-green-700 dark:text-green-400 rounded-xl border border-green-200 dark:border-green-900/30 flex items-center gap-3 animate-in fade-in slide-in-from-top-2">
...
<?php if (!empty($error)): ?>
    <div class="mb-6 p-4 bg-red-50 dark:bg-red-900/20 text-red-700 dark:text-red-400 rounded-xl border border-red-200 dark:border-red-900/30 flex items-center gap-3 animate-in fade-in slide-in-from-top-2">
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
    <!-- Profile Card -->
    <div class="bg-white dark:bg-white/5 rounded-2xl p-8 border border-gray-200 dark:border-white/10 shadow-sm relative overflow-hidden group">
        <div class="absolute top-0 left-0 w-full h-2 bg-gradient-to-r from-primary to-purple-600"></div>
        
        <div class="flex flex-col items-center text-center">
            <div class="relative mb-6">
                <div class="w-24 h-24 rounded-2xl bg-gradient-to-br from-primary to-purple-600 flex items-center justify-center text-white font-black text-4xl shadow-lg ring-4 ring-white dark:ring-white/5">
                    <?php echo strtoupper(substr($user['full_name'], 0, 1)); ?>
                </div>
                <div class="absolute -bottom-2 -right-2 w-8 h-8 bg-accent-green rounded-full border-4 border-white dark:border-background-dark flex items-center justify-center shadow-sm">
                    <span class="material-symbols-outlined text-white text-xs">verified</span>
                </div>
            </div>
            
            <h3 class="text-2xl font-black text-[#0f0e1b] dark:text-white mb-1 group-hover:text-primary transition-colors">
            ...
            <div class="w-full space-y-4 pt-6 border-t border-gray-100 dark:border-white/5 text-sm">
                ...
                <div class="flex justify-between items-center">
                    ...
                    <span class="px-3 py-1 bg-green-50 text-green-700 dark:bg-green-900/20 dark:text-green-400 rounded-full text-xs font-black border border-green-100 dark:border-green-900/30">
    ...
    <div class="lg:col-span-2">
        <form method="POST" class="bg-white dark:bg-white/5 rounded-2xl p-8 border border-gray-200 dark:border-white/10 shadow-sm">
...
<script>
    ...
    const iti = window.intlTelInput(phoneInput, {
        ...
        geoIpLookup: function (callback) {
            fetch('https://ipinfo.io/json?token=7ed42340277884')
